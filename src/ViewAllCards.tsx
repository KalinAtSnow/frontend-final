...
export function ViewAllCards() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await apiService.GetRange(7,25)
                setCardData(data) 
            } catch (error) {
    ...
    return (
        ...
        <div className="p-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mx-auto lg:grid-cols-6 gap-4">
          {cardData?.map((card) => (
            <div className="" key={card.id} onClick={() => cardClicked(card.id)}>
              <img className="hover:scale-105 hover:shadow-lg" src={card.imageurl} />
            </div>
          ))}
          </div>
          </div>
